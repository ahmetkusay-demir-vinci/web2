...
router.get('/', (req, res, next) => {
  ...
  const filmsParse = parse(jsonDbPath, films); //la fonction parse permet la lecture
                                               // CTRL + click droit sur parse pour voir ce que c'est
...
router.get('/:id', (req, res) => {
  ...
  res.json(filmsParse[indexOfFilmFound]);
});
...
router.post('/', (req, res) => {
  ...
  const existingFilm = filmsParse.find(
...
router.delete('/:id', (req, res) => {
  ...
  const filmsParse = parse(jsonDbPath, films);
  const foundIndex = filmsParse.findIndex(film => film.id == req.params.id);  // On récupère l'élément qu'on veut effacer et si on trouve pas d'élément avec l'identifiant passé en paramètre , la méthode renvoie -1

  if (foundIndex < 0) return res.sendStatus(404);

  const itemsRemovedFromFilms = filmsParse.splice(foundIndex, 1); // splice effacera 1 élément à partie de l'index trouvé 
  const itemRemoved = itemsRemovedFromFilms[0]; /* splice renvoie un tableau de tt les éléments supprimés , Pour ça on cherche le 1 er élément dans le tableau et y en aura que 1 vu que l'id est unique */
...
router.put('/:id', function (req, res) {
  ...
  const filmsParse = parse(jsonDbPath, films);
  const indexOfFilmFound = filmsParse.findIndex((film) => film.id == id);
